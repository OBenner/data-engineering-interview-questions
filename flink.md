1. See you using Flink, briefly introduce Flink?
...
2. What are the differences between Flink and Spark Streaming?
...
3. What are the Flink component stacks
...
4. Does Flink need to depend on Hadoop?
...
6. What is the Flink programming model?
   In fact, in one sentence,Source->Transformation->Sink*

7. What are the roles of Flink cluster? What are the functions?
...
9. What are the commonly used operators in Flink?
   Map operator
   Filter operator
   KeyBy operator
   Window window
10. What is the parallelism of Flink? What is the parallelism setting of Flink?
...
11. What is the relationship between Flink's Solt and parallelism?
...
12. What if Flink encounters an abnormal restart of the program?
...
14. Broadcast variables in Flink
...
15. Do you know what windows in Flink are?
...
FsSateBackend
RocksDBStateBackend
17. What kind of time are there in Flink?
18. Event time: the time when the event actually occurred

19. Intake time: time to enter flink

20. Processing time: the time to enter the flink operator

21. What is Watermark in Flink?
...
. What is the list of top operations that we can perform by using Apache Flink?
...
3、Flink What can I do with my state ？
Flink There are two main ways to use state ：

checkpoint Data recovery
Logical computing
4、Flink Of waterMark Mechanism ,Flink watermark Transmission mechanism
...
7、flink What are the window functions
Tumbing window
...
8、keyedProcessFunction How it works . If so event time Words
...
9、flink How to deal with offline data, such as the association with offline data ？
1、async io
...
10、flink Supported data types
DataSet Api and DataStream Api、Table Api

11、Flink What if there is a data skew
...
12、flink How to do dimension table Association
1、async io
...
13、Flink checkpoint The overtime problem of How to solve .
...
14、flinkTopN And offline TopN The difference between
...
15、sparkstreaming and flink in checkpoint The difference between
...
16、 A brief introduction cep State programming
...
26、Flink in state What are the storage methods
Memery、RocksDB、HDFS

27、Flink How to deal with abnormal data
...
28、Flink Monitor how you do it
1、 We monitored Flink Whether or not the task of

2、 We monitored Flink Of Kafka Of LAG

3、 We'll do real-time data reconciliation , Like sales .

29、Flink Is there any possibility of data loss
...
30、Flink interval join Can you write it simply
...
31、Flink At the time of submission How to make parallelism , And how to allocate resources
Parallelism based on kafka topic Parallelism of , A degree of parallelism 3 individual G

32、Flink Of boardcast join What is the principle of
utilize broadcast State Broadcast the dimension data stream to all downstream task in . This broadcast The flow of can interact with our event flow connect, And then in the following process Operator in the association operation .

33、flink Of source The end is broken , such as kafka Something's wrong , No data sent , How to deal with ？
There will be a call to the police , Monitored kafka The offset is LAG.

34、flink What's the common stream of API?
window join ah cogroup ah map flatmap,async io etc.

35、flink The water level line of , Do you understand? , Can you give me a brief introduction
...
36、Flink How to maintain Checkpoint？ stay HDFS Will there be small files stored on the Internet
...
37、Spark and Flink Serialization , What's the difference ？
...
38、Flink How to deal with late data ？ But in the actual development, no data can be late , How do you do it? ？
Flink Of watermark It's a delayed trigger mechanism .
...
46、flink Calculated UV How do you set the state back end to save data
You can use a bloom filter .

Flink The restart strategy of
Fixed delay restart strategy

The fixed delay restart strategy is to try to restart the job for a given number of times . If the maximum number of attempts is exceeded , The job fails . Between two consecutive restart attempts , There will be a fixed delay waiting time .

Failure rate restart strategy

The failure rate restart strategy is to restart after failure , When setting the failure rate （failure rate） Failure beyond each time interval , The job failed in the end . Between two consecutive restart attempts , Restart policy delay wait for a while .
...
Use the cluster defined restart policy . This is helpful for streaming programs that enable checkpointing . By default , If no other restart policy is defined , Select the fixed delay restart policy .

60、Flink When to use aggregate() perhaps process()
